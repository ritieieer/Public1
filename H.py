# H.py - MODIFIED VERSION WITHOUT APPROVAL & WITH SERVER CONTROL
import telebot
import subprocess
import os
import zipfile
...
# NO LIMITS - UNLIMITED FILES FOR ALL
...
bot_locked = False
server_running = False  # Track if all server scripts are running
...
ADMIN_COMMAND_BUTTONS_LAYOUT_USER_SPEC = [
    ...
    ["🔒 Lock Bot", "🟢 Start Server"],
    ["🔴 Stop Server", "👑 Admin Panel"],
    ["📞 Contact Owner", "🤖 MPX Ai"],
    ["⏱ Uptime", "🔄 Restart Server"]
]
...
def create_main_menu_inline(user_id):
    ...
    if user_id in admin_ids:
        admin_buttons = [
            ...
            types.InlineKeyboardButton('🟢 Start Server', callback_data='start_server'),
            types.InlineKeyboardButton('🔴 Stop Server', callback_data='stop_server'),
            types.InlineKeyboardButton('🔄 Restart Server', callback_data='restart_server')
        ]
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        # AUTO START THE SCRIPT AFTER UPLOAD
        file_path = os.path.join(user_folder, main_script_name)
        if file_type == 'py':
            threading.Thread(target=run_script, args=(file_path, user_id, user_folder, main_script_name, message)).start()
        elif file_type == 'js':
            threading.Thread(target=run_js_script, args=(file_path, user_id, user_folder, main_script_name, message)).start()
        
        logger.info(f"Saved and auto-started main script '{main_script_name}' ({file_type}) for {user_id} from zip.")
...
def start_all_server_scripts(message_obj_for_reply):
    """Start all user scripts on the server"""
    global server_running
    if server_running:
        bot.reply_to(message_obj_for_reply, "✅ Server is already running!")
        return
    
    bot.reply_to(message_obj_for_reply, "🟢 Starting all server scripts...")
    logger.info(f"Starting all server scripts by {message_obj_for_reply.from_user.id}")
    ...
    for target_user_id, files_for_user in all_user_files_snapshot.items():
        ...
        total_files += len(files_for_user)
            ...
            if not is_bot_running(target_user_id, file_name):
                ...
                if os.path.exists(file_path):
                    try:
                        ...
                        logger.error(f"Error starting '{file_name}' for user {target_user_id}: {e}")
    
    server_running = True
    bot.reply_to(message_obj_for_reply, 
                f"✅ Server started successfully!\n"
                f"📊 Started: {started_count}/{total_files} scripts\n"
                f"👥 Users: {len(all_user_files_snapshot)}\n"
                f"🟢 Server Status: RUNNING")

def stop_all_server_scripts(message_obj_for_reply):
    """Stop all user scripts on the server"""
    global server_running
    if not server_running:
        bot.reply_to(message_obj_for_reply, "✅ Server is already stopped!")
        return
    
    bot.reply_to(message_obj_for_reply, "🔴 Stopping all server scripts...")
    logger.info(f"Stopping all server scripts by {message_obj_for_reply.from_user.id}")
    ...
    bot.reply_to(message_obj_for_reply, 
                ...
                f"📊 Stopped: {stopped_count} scripts\n"
                f"🔴 Server Status: STOPPED")

def restart_all_server_scripts(message_obj_for_reply):
    """Restart all user scripts on the server"""
    global server_running
    
    bot.reply_to(message_obj_for_reply, "🔄 Restarting all server scripts...")
    logger.info(f"Restarting all server scripts by {message_obj_for_reply.from_user.id}")
        ...
        for file_name, file_type in files_for_user:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    logger.error(f"Error restarting '{file_name}' for user {target_user_id}: {e}")
...
def _logic_send_welcome(message):
    ...
    limit_str = "Unlimited"  # Always unlimited now
    ...
    welcome_msg_text = (f"Welcome, {user_name}!\n\nYour User ID: `{user_id}`\n"
                        ...
                        f"🚀 **NO LIMITS:** Upload unlimited files!\n"
                        f"⚡ **AUTO-START:** Files start automatically!\n\n"
...
def _logic_upload_file(message):
    ...
    bot.reply_to(message, 
                ...
                "🚀 **Files will auto-start immediately!**\n"
                "📊 **No limits - Upload unlimited files!**")
...
def _logic_check_files(message):
    ...
    for file_name, file_type in sorted(user_files_list):
        ...
        response += f"{status_icon} `{file_name}` - {status_text}\n"
...
def _logic_bot_speed(message):
    ...
    try:
        ...
        speed_msg = (f"Bot Speed & Status:\n\nAPI Response Time: {response_time} ms\n"
                     ...
                     f"Server Status: {'🟢 RUNNING' if server_running else '🔴 STOPPED'}")
...
def _logic_uptime(message):
    ...
    bot.reply_to(message, f"Bot Uptime: `{uptime_str}`\nServer Status: {'🟢 RUNNING' if server_running else '🔴 STOPPED'}", parse_mode='Markdown')
...
def _logic_statistics(message):
    ...
    if user_id in admin_ids:
        stats_msg += f"\n👑 **Admin Stats:**\n"
        stats_msg += f"Your Running Bots: {user_running_bots}\n"
        stats_msg += f"Total Running Scripts: {len(bot_scripts)}"
    else:
        stats_msg += f"\n👤 **Your Stats:**\n"
        stats_msg += f"Your Files: {get_user_file_count(user_id)}\n"
        stats_msg += f"Your Running Bots: {user_running_bots}"
...
def _logic_start_server(message):
    if message.from_user.id != OWNER_ID:
        bot.reply_to(message, "⚠️ Only Owner can control server!")
        return
    start_all_server_scripts(message)

def _logic_stop_server(message):
    if message.from_user.id != OWNER_ID:
        bot.reply_to(message, "⚠️ Only Owner can control server!")
        return
    stop_all_server_scripts(message)

def _logic_restart_server(message):
    if message.from_user.id != OWNER_ID:
        bot.reply_to(message, "⚠️ Only Owner can control server!")
        return
    restart_all_server_scripts(message)
...
def command_start_server(message):
    _logic_start_server(message)

@bot.message_handler(commands=['stopserver'])
def command_stop_server(message):
    _logic_stop_server(message)

@bot.message_handler(commands=['restartserver'])
def command_restart_server(message):
    _logic_restart_server(message)
...
def command_server_status(message):
    ...
    status_msg = (f"📊 **Server Status:**\n\n"
                  ...
                  f"📁 Total Scripts: {len(bot_scripts)}\n"
                  f"🟢 Active Scripts: {running_count}\n"
                  f"👥 Total Users: {len(user_files)}\n"
                  f"📂 Total Files: {sum(len(files) for files in user_files.values())}")
    
    if user_id == OWNER_ID:
        status_msg += f"\n\n👑 **Owner Controls:**\n"
        status_msg += f"Use /startserver to start all\n"
        status_msg += f"Use /stopserver to stop all\n"
        status_msg += f"Use /restartserver to restart all"
...
BUTTON_TEXT_TO_LOGIC = {
    ...
    "🟢 Start Server": _logic_start_server,
    "🔴 Stop Server": _logic_stop_server,
    "🔄 Restart Server": _logic_restart_server,
...
def handle_file_upload_doc(message):
    ...
    # NO FILE LIMIT CHECK - UNLIMITED FOR EVERYONE
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Failed to notify owner about uploaded file: {e}")
...
def handle_callbacks(call):
    ...
    try:
        if data == 'upload':
            ...
            if user_id != OWNER_ID:
                bot.answer_callback_query(call.id, "⚠️ Only Owner can start server!", show_alert=True)
                return
            start_all_server_scripts(call.message)
            bot.answer_callback_query(call.id, "Starting server...")
        elif data == 'stop_server':
            if user_id != OWNER_ID:
                bot.answer_callback_query(call.id, "⚠️ Only Owner can stop server!", show_alert=True)
                return
            stop_all_server_scripts(call.message)
            bot.answer_callback_query(call.id, "Stopping server...")
        elif data == 'restart_server':
            if user_id != OWNER_ID:
                bot.answer_callback_query(call.id, "⚠️ Only Owner can restart server!", show_alert=True)
                return
            restart_all_server_scripts(call.message)
            bot.answer_callback_query(call.id, "Restarting server...")
        elif data == 'broadcast':
            ...
            bot.answer_callback_query(call.id)
            uptime_str = get_uptime()
            bot.send_message(call.message.chat.id, f"Bot Uptime: `{uptime_str}`\nServer Status: {'🟢 RUNNING' if server_running else '🔴 STOPPED'}", parse_mode='Markdown')
        else:
...
def speed_callback(call):
    ...
    try:
        ...
        bot.edit_message_text(speed_msg, chat_id, call.message.message_id, 
         ...
         try: 
             bot.edit_message_text("Main Menu", chat_id, call.message.message_id, 
                                  reply_markup=create_main_menu_inline(user_id))
         except Exception: 
             pass
...
def back_to_main_callback(call):
    ...
    main_menu_text = (f"Welcome back, {call.from_user.first_name}!\n\n"
                      ...
                      f"🚀 Server: {'🟢 RUNNING' if server_running else '🔴 STOPPED'}{admin_info}\n\n"
                      f"Use buttons or type commands.")
    try:
        bot.answer_callback_query(call.id)
