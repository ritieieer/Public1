        ...
        for uid in active_users:
            try:
                bot.send_message(
                    ...
                    "Server is under maintenance. Some features may be limited."
                )
            except:
                pass
...
@bot.message_handler(func=lambda msg: msg.text == "🟢 Run All")
def run_all_command(message):
    user_id = message.from_user.id
    ...
    # Implementation for running all scripts
    # (You can add your existing run_all_scripts logic here)
...
def create_maintenance_script():
    """Create maintenance script to run when maintenance mode is on"""
...
print("For updates, check our channel.")
print("=" * 50)
'''
    return script

# ====================== SCRIPT RUNNING FUNCTIONS ======================
def run_user_script(user_id, file_name):
    """Run user's script (with maintenance check)"""
    if maintenance_mode:
        # Run maintenance script instead
        script_content = create_maintenance_script()
        temp_file = os.path.join(get_user_folder(user_id), "maintenance.py")
    ...
    # Check file type and run accordingly
